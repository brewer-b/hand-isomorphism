...
int main(int argc, char **argv) {
  ...
  HandIsomorphism iso{{{2}, {2, 3}, {2, 3, 1}, {2, 3, 1, 1}}};

  printf("sizes:"
         " %" PRIhand_index " %" PRIhand_index " %" PRIhand_index
         " %" PRIhand_index "\n",
         iso.size(0), iso.size(1), iso.size(2), iso.size(3));

  assert(iso.size(0) == 169);
  assert(iso.size(1) == 1286792);
  assert(iso.size(2) == 55190538);
  assert(iso.size(3) == 2428287420);
  ...
  for (uint_fast32_t i = 0; i < RANKS; ++i) {
    ...
    for (uint_fast32_t j = 0; j < RANKS; ++j) {
      ...
      hand_index_t index = iso.index(0, cards);
