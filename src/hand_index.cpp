...
static card_t deck_get_suit(card_t card) {
  return card&3;
}

static card_t deck_get_rank(card_t card) {
  return card>>2;
}

static card_t deck_make_card(card_t suit, card_t rank) {
  return rank<<2 | suit;
}
...
void HandIndexer::hand_index_ctor() {
    // Dynamic memory allocation for large arrays
    nth_unset = new uint8_t[1 << RANKS][RANKS];
    equal = new bool[1 << (SUITS - 1)][SUITS];
    nCr_ranks = new uint_fast32_t[RANKS + 1][RANKS + 1];
    rank_set_to_index = new uint_fast32_t[1 << RANKS];
    index_to_rank_set = new uint_fast32_t[RANKS + 1][1 << RANKS];
    nCr_groups = new hand_index_t[MAX_GROUP_INDEX][SUITS + 1];

    // Initialize the equal array
    for (uint_fast32_t i = 0; i < (1 << (SUITS - 1)); ++i) {
        for (uint_fast32_t j = 1; j < SUITS; ++j) {
            equal[i][j] = (i & (1 << (j - 1))) != 0;
        }
    }
    // Initialize the nth_unset array
    ...
    // Compute nCr_ranks (combinations)
    nCr_ranks[0][0] = 1;
    for (uint_fast32_t i = 1; i <= RANKS; ++i) {
    ...
    // Initialize rank_set_to_index and index_to_rank_set
    for (uint_fast32_t i = 0; i < (1 << RANKS); ++i) {
        rank_set_to_index[i] = 0;
    ...
    // Allocate memory for suit_permutations
    suit_permutations = static_cast<uint_fast32_t (*)[SUITS]>(calloc(num_permutations, SUITS * sizeof(uint_fast32_t)));
    if (!suit_permutations) {
        std::cerr << "Memory allocation failed for suit_permutations\n";
        return;
    }
    // Initialize suit_permutations
    for (uint_fast32_t i = 0; i < num_permutations; ++i) {
        uint_fast32_t index = i;
        uint_fast32_t used = 0;
        for (uint_fast32_t j = 0; j < SUITS; ++j) {
...
void HandIndexer::hand_index_dtor() {

    // Free dynamically allocated memory
    delete[] nth_unset;
    delete[] equal;
    delete[] nCr_ranks;
    delete[] rank_set_to_index;
    delete[] index_to_rank_set;
    delete[] nCr_groups;
    free(suit_permutations);

}

void HandIndexer::enumerate_configurations_r(uint_fast32_t rounds, const uint8_t cards_per_round[], uint_fast32_t round, uint_fast32_t remaining, uint_fast32_t suit, uint_fast32_t equal, uint_fast32_t used[], uint_fast32_t configuration[], std::function<void(uint_fast32_t, uint_fast32_t*, void*)> observe, void * data){
  if (suit == SUITS) {
...
void HandIndexer::enumerate_configurations(uint_fast32_t rounds, const uint8_t *cards_per_round, std::function<void(uint_fast32_t, uint_fast32_t*, void*)> observe, void* data){
...
void HandIndexer::count_configurations(uint_fast32_t round, uint_fast32_t configuration[], void * data) {
  uint_fast32_t * counts = static_cast<uint_fast32_t*>(data); ++counts[round];
}

void HandIndexer::tabulate_configurations(uint_fast32_t round, uint_fast32_t configuration[], void * data) {
  hand_indexer_t* indexer = static_cast<hand_indexer_t*>(data);
...
void HandIndexer::enumerate_permutations_r(uint_fast32_t rounds, const uint8_t cards_per_round[], uint_fast32_t round, uint_fast32_t remaining, uint_fast32_t suit, uint_fast32_t used[], uint_fast32_t count[], std::function<void(uint_fast32_t, uint_fast32_t*, void*)> observe, void * data) {
  if (suit == SUITS) {
...
void HandIndexer::enumerate_permutations(uint_fast32_t rounds, const uint8_t cards_per_round[], std::function<void(uint_fast32_t, uint_fast32_t*, void*)> observe, void * data){
...
void HandIndexer::count_permutations(uint_fast32_t round, uint_fast32_t count[], void * data) {
  hand_indexer_t * indexer = static_cast<hand_indexer_t*>(data);
...
void HandIndexer::tabulate_permutations(uint_fast32_t round, uint_fast32_t count[], void * data) {
  hand_indexer_t * indexer = static_cast<hand_indexer_t*>(data);
  ...
  while(low < high) {
    ...
    for(uint_fast32_t i=0; i<SUITS; ++i) {
      uint_fast32_t _this  = count[pi[i]];
...
bool HandIndexer::hand_indexer_init(uint_fast32_t rounds, const uint8_t cards_per_round[], hand_indexer_t * indexer) {
  if (rounds == 0) {
    return false;
  }
  ...
  auto count_configurations_lambda = [this](uint_fast32_t round, uint_fast32_t* configuration, void* data) {
        this->count_configurations(round, configuration, data);
    };
  enumerate_configurations(rounds, cards_per_round, count_configurations_lambda, indexer->configurations);

  for(uint_fast32_t i=0; i<rounds; ++i) {
    indexer->configuration_to_equal[i]     = static_cast<uint_fast32_t*>(calloc(indexer->configurations[i], sizeof(uint_fast32_t)));
    indexer->configuration_to_offset[i]    = static_cast<hand_index_t*>(calloc(indexer->configurations[i], sizeof(hand_index_t)));
    indexer->configuration[i]              = static_cast<uint_fast32_t (*)[SUITS]>(calloc(indexer->configurations[i], SUITS * sizeof(uint_fast32_t)));
    indexer->configuration_to_suit_size[i] = static_cast<uint_fast32_t (*)[SUITS]>(calloc(indexer->configurations[i], SUITS * sizeof(uint_fast32_t)));
  ...
  memset(indexer->configurations, 0, sizeof(indexer->configurations));
  auto tabulate_configurations_lambda = [this](uint_fast32_t round, uint_fast32_t* configuration, void* data) {
        this->tabulate_configurations(round, configuration, data);
    };
  enumerate_configurations(rounds, cards_per_round, tabulate_configurations_lambda, indexer);
  ...
  auto count_permutations_lambda = [this](uint_fast32_t round, uint_fast32_t* permutation, void* data) {
        this->count_permutations(round, permutation, data);
    };
  enumerate_permutations(rounds, cards_per_round, count_permutations_lambda, indexer);
  
  for(uint_fast32_t i=0; i<rounds; ++i) {
    indexer->permutation_to_configuration[i] = static_cast<uint_fast32_t*>(calloc(indexer->permutations[i], sizeof(uint_fast32_t)));
    indexer->permutation_to_pi[i] = static_cast<uint_fast32_t*>(calloc(indexer->permutations[i], sizeof(uint_fast32_t)));
  ...
  auto tabulate_permutations_lambda = [this](uint_fast32_t round, uint_fast32_t* permutation, void* data) {
    this->tabulate_permutations(round, permutation, data);
  };
  enumerate_permutations(rounds, cards_per_round, tabulate_permutations_lambda, indexer);

  return true;
}

void HandIndexer::hand_indexer_free(hand_indexer_t * indexer) {
...
hand_index_t HandIndexer::hand_indexer_size(const hand_indexer_t * indexer, uint_fast32_t round) {
  assert(round < indexer->rounds);
  return indexer->round_size[round];
}

void HandIndexer::hand_indexer_state_init(const hand_indexer_t * indexer, hand_indexer_state_t * state){
...
hand_index_t HandIndexer::hand_index_all(const hand_indexer_t * indexer, const uint8_t cards[], hand_index_t indices[]) {
  if (indexer->rounds) {
...
hand_index_t HandIndexer::hand_index_last(const hand_indexer_t * indexer, const uint8_t cards[]) {
  hand_index_t indices[MAX_ROUNDS];
  return hand_index_all(indexer, cards, indices);
}

hand_index_t HandIndexer::hand_index_next_round(const hand_indexer_t * indexer, const uint8_t cards[], hand_indexer_state_t * state) {
...
bool HandIndexer::hand_unindex(const hand_indexer_t * indexer, uint_fast32_t round, hand_index_t index, uint8_t cards[]) {
