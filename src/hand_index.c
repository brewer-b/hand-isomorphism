...
#ifdef _MSC_VER
#include "intrin.h"
static inline uint32_t count_trailing_zeros(uint32_t value) {
    unsigned long index;
...
indexer_helper_t* indexer_helper_ctor() {
    ...
    poker_data->nCr_ranks = calloc(RANKS + 1, (RANKS + 1) * sizeof(uint32_t));
    poker_data->rank_set_to_index = calloc((1 << RANKS), sizeof(uint32_t));
    poker_data->index_to_rank_set = calloc(RANKS + 1, (1 << RANKS) * sizeof(uint32_t));
    poker_data->nCr_groups = calloc(MAX_GROUP_INDEX, (SUITS + 1) * sizeof(hand_index_t));

    uint32_t num_permutations = 1;
    for (uint32_t i = 2; i <= SUITS; ++i) {
        num_permutations *= i;
    }
    poker_data->suit_permutations = (uint32_t (*)[SUITS])calloc(num_permutations, SUITS * sizeof(uint32_t));
    ...
    for (uint32_t i = 0; i < 1 << (SUITS - 1); ++i) {
        for (uint32_t j = 1; j < SUITS; ++j) {
            poker_data->equal[i][j] = i & (1 << (j - 1));
        }
    }

    for (uint32_t i = 0; i < 1 << RANKS; ++i) {
        for (uint32_t j = 0, set = ~i & ((1 << RANKS) - 1); j < RANKS; ++j, set &= set - 1) {
            poker_data->nth_unset[i][j] = set ? __builtin_ctz(set) : 0xff;
        }
    }

    poker_data->nCr_ranks[0][0] = 1;
    for (uint32_t i = 1; i < RANKS + 1; ++i) {
    ...
    for (uint32_t i = 1; i < MAX_GROUP_INDEX; ++i) {
        ...
        for (uint32_t j = 1; j < (i < (SUITS + 1) ? i : (SUITS + 1)); ++j) {
    ...
    for (uint32_t i = 0; i < num_permutations; ++i) {
        for (uint32_t j = 0, index = i, used = 0; j < SUITS; ++j) {
            uint32_t suit = index % (SUITS - j);
            index /= SUITS - j;
            uint32_t shifted_suit = poker_data->nth_unset[used][suit];
...
void indexer_helper_dtor(indexer_helper_t* poker_data) {
    if (!poker_data) return;

    free(poker_data->nth_unset);
    free(poker_data->equal);
    free(poker_data->nCr_ranks);
    free(poker_data->rank_set_to_index);
    free(poker_data->index_to_rank_set);
    free(poker_data->nCr_groups);
    free(poker_data->suit_permutations);

    poker_data->nth_unset = NULL;
    poker_data->equal = NULL;
    poker_data->nCr_ranks = NULL;
    poker_data->rank_set_to_index = NULL;
...
void enumerate_configurations_r(const indexer_helper_t* poker_data, uint32_t rounds, const uint8_t cards_per_round[], 
    uint32_t round, uint32_t remaining, 
    uint32_t suit, uint32_t equal, uint32_t used[], uint32_t configuration[],
    void (*observe)(const indexer_helper_t*, uint32_t, uint32_t[], void*), void * data) {
  if (suit == SUITS) {
    observe(poker_data, round, configuration, data);

    if (round+1 < rounds) {
      enumerate_configurations_r(poker_data, rounds, cards_per_round, round+1, cards_per_round[round+1], 0, equal, used, configuration, observe, data);
    }
  } else {
    uint32_t min = 0;
    if (suit == SUITS-1) {
    ...
    uint32_t max = RANKS-used[suit];
    if (remaining < max) {
      max = remaining;
    }
   
    uint32_t previous = RANKS+1;
    ...
    for(uint32_t i=min; i<=max; ++i) {
      uint32_t new_configuration = old_configuration | i<<ROUND_SHIFT*(rounds-round-1);
      uint32_t new_equal = (equal&~(1<<suit))|(was_equal&(i==previous))<<suit;

      used[suit] = old_used+i;
      configuration[suit] = new_configuration;
      enumerate_configurations_r(poker_data, rounds, cards_per_round, round, remaining-i, suit+1, new_equal, used, configuration, observe, data);
...
void enumerate_configurations(const indexer_helper_t* poker_data, uint32_t rounds, const uint8_t cards_per_round[],
    void (*observe)(const indexer_helper_t*, uint32_t, uint32_t[], void*), void * data) {
  uint32_t used[SUITS] = {0}, configuration[SUITS] = {0};
  enumerate_configurations_r(poker_data, rounds, cards_per_round, 0, cards_per_round[0], 0, (1<<SUITS) - 2, used, configuration, observe, data);
}

void count_configurations(const indexer_helper_t* poker_data, uint32_t round, uint32_t configuration[], void * data) {
  uint32_t * counts = data; ++counts[round];
}

void tabulate_configurations(const indexer_helper_t* poker_data, uint32_t round, uint32_t configuration[], void * data) {
...
out:;
  ...
  uint32_t equal = 0;
  for(uint32_t i=0; i<SUITS;) {
    hand_index_t size = 1;
    for(uint32_t j=0, remaining=RANKS; j<=round; ++j) {
      uint32_t ranks = configuration[i]>>ROUND_SHIFT*(indexer->rounds-j-1)&ROUND_MASK;
      size *= poker_data->nCr_ranks[remaining][ranks];
      remaining -= ranks;
    }
    assert(size+SUITS-1 < MAX_GROUP_INDEX);
    
    uint32_t j=i+1; for(; j<SUITS && configuration[j] == configuration[i]; ++j) {} 
    for(uint32_t k=i; k<j; ++k) {
      indexer->configuration_to_suit_size[round][id][k] = size;
    }

    indexer->configuration_to_offset[round][id] *= poker_data->nCr_groups[size+j-i-1][j-i];
...
void enumerate_permutations_r(uint32_t rounds, const uint8_t cards_per_round[], 
    uint32_t round, uint32_t remaining, 
    uint32_t suit, uint32_t used[], uint32_t count[],
    void (*observe)(uint32_t, uint32_t[], void*), void * data) {
  if (suit == SUITS) {
...
void enumerate_permutations(uint32_t rounds, const uint8_t cards_per_round[],
    void (*observe)(uint32_t, uint32_t[], void*), void * data) {
  uint32_t used[SUITS] = {0}, count[SUITS] = {0};
...
void count_permutations(uint32_t round, uint32_t count[], void * data) {
  ...
  for(uint32_t i=0; i<=round; ++i) {
    for(uint32_t j=0, remaining=indexer->cards_per_round[i]; j<SUITS-1; ++j) {
      uint32_t size = count[j]>>(indexer->rounds-i-1)*ROUND_SHIFT&ROUND_MASK;
      idx  += mult*size;
      mult *= remaining+1;
      remaining -= size;
    }
...
void tabulate_permutations(uint32_t round, uint32_t count[], void * data) {
  ...
  for(uint32_t i=1; i<SUITS; ++i) {
    uint32_t j=i, pi_i = pi[i]; for(; j>0; --j) {
  ...
  for(uint32_t i=0; i<SUITS; ++i) {
    ...
    uint32_t smaller  = __builtin_popcount((this_bit-1)&pi_used);
...
hand_indexer_t* hand_indexer_init(const indexer_helper_t* poker_data, uint32_t rounds, const uint8_t cards_per_round[]) {
  ...
  enumerate_configurations(poker_data, rounds, cards_per_round, count_configurations, indexer->configurations);

  for(uint32_t i=0; i<rounds; ++i) {
    indexer->configuration_to_equal[i]     = calloc(indexer->configurations[i], sizeof(uint32_t));
    indexer->configuration_to_offset[i]    = calloc(indexer->configurations[i], sizeof(hand_index_t));
    indexer->configuration[i]              = calloc(indexer->configurations[i], SUITS*sizeof(uint32_t));
    indexer->configuration_to_suit_size[i] = calloc(indexer->configurations[i], SUITS*sizeof(uint32_t));
  ...
  memset(indexer->configurations, 0, sizeof(indexer->configurations));
  enumerate_configurations(poker_data, rounds, cards_per_round, tabulate_configurations, indexer);
  
  for(uint32_t i=0; i<rounds; ++i) {
    hand_index_t accum = 0; for(uint32_t j=0; j<indexer->configurations[i]; ++j) {
  ...
  for(uint32_t i=0; i<rounds; ++i) {
    indexer->permutation_to_configuration[i] = calloc(indexer->permutations[i], sizeof(uint32_t));
    indexer->permutation_to_pi[i] = calloc(indexer->permutations[i], sizeof(uint32_t));
...
hand_index_t hand_index_all(const indexer_helper_t* poker_data, const hand_indexer_t * indexer, const uint8_t cards[], uint64_t indices[]) {
  if (indexer->rounds) {
    ...
    for(uint32_t i=0, j=0; i<indexer->rounds; j+=indexer->cards_per_round[i++]) {
      indices[i] = hand_index_next_round(poker_data, indexer, cards+j, &state);
    }
...
hand_index_t hand_index_last(const indexer_helper_t* poker_data, const hand_indexer_t * indexer, const uint8_t cards[]) {
  ...
  return hand_index_all(poker_data, indexer, cards, indices);
}
...
hand_index_t hand_index_next_round(const indexer_helper_t* poker_data, const hand_indexer_t * indexer, const uint8_t cards[], hand_indexer_state_t * state) {
  ...
  uint32_t ranks[SUITS] = {0}, shifted_ranks[SUITS] = {0};
  for(uint32_t i=0; i<indexer->cards_per_round[round]; ++i) {
    assert(cards[i] < CARDS);                 /* valid card */

    uint32_t rank         = deck_get_rank(cards[i]), suit = deck_get_suit(cards[i]), rank_bit = 1<<rank;
  ...
  for(uint32_t i=0; i<SUITS; ++i) {
    ...
    uint32_t used_size    = __builtin_popcount(state->used_ranks[i]), this_size = __builtin_popcount(ranks[i]);
    state->suit_index[i]      += state->suit_multiplier[i]*poker_data->rank_set_to_index[shifted_ranks[i]];
    state->suit_multiplier[i] *= poker_data->nCr_ranks[RANKS-used_size][this_size];
    state->used_ranks[i]      |= ranks[i];
  }

  for(uint32_t i=0, remaining=indexer->cards_per_round[round]; i<SUITS-1; ++i) {
    uint32_t this_size          = __builtin_popcount(ranks[i]);
    state->permutation_index        += state->permutation_multiplier*this_size;
    state->permutation_multiplier   *= remaining+1;
    remaining                       -= this_size;
  }

  uint32_t configuration = indexer->permutation_to_configuration[round][state->permutation_index];
  uint32_t pi_index      = indexer->permutation_to_pi[round][state->permutation_index];
  ...
  const uint32_t * pi    = poker_data->suit_permutations[pi_index];
...
#define swap(u, v) \
  ...
  for(uint32_t i=0; i<SUITS;) {
    ...
    if (i+1 < SUITS && poker_data->equal[equal_index][i+1]) {
      if (i+2 < SUITS && poker_data->equal[equal_index][i+2]) {
        if (i+3 < SUITS && poker_data->equal[equal_index][i+3]) {
          /* four equal suits */
          swap(i, i+1); swap(i+2, i+3); swap(i, i+2); swap(i+1, i+3); swap(i+1, i+2);
          part = suit_index[i] + poker_data->nCr_groups[suit_index[i+1]+1][2] + poker_data->nCr_groups[suit_index[i+2]+2][3] + poker_data->nCr_groups[suit_index[i+3]+3][4];
          size = poker_data->nCr_groups[suit_multiplier[i]+3][4];
          i += 4;
        } else {
          /* three equal suits */
          swap(i, i+1); swap(i, i+2); swap(i+1, i+2);
          part = suit_index[i] + poker_data->nCr_groups[suit_index[i+1]+1][2] + poker_data->nCr_groups[suit_index[i+2]+2][3];
          size = poker_data->nCr_groups[suit_multiplier[i]+2][3];
          i += 3;
        }
      } else {
        /* two equal suits*/
        swap(i, i+1);
        part = suit_index[i] + poker_data->nCr_groups[suit_index[i+1]+1][2];
        size = poker_data->nCr_groups[suit_multiplier[i]+1][2];
        i += 2;
      }
    } else {
      /* no equal suits */
...
bool hand_unindex(const indexer_helper_t* poker_data, const hand_indexer_t * indexer, uint32_t round, uint64_t index, uint8_t cards[]) {
  ...
  uint32_t low = 0, high = indexer->configurations[round], configuration_idx = 0;
  while(low < high) {
    uint32_t mid = (low+high)/2;
  ...
  for(uint32_t i=0; i<SUITS;) {
    uint32_t j=i+1; for(; j<SUITS && indexer->configuration[round][configuration_idx][j] == indexer->configuration[round][configuration_idx][i]; ++j) {}
    
    uint32_t suit_size  = indexer->configuration_to_suit_size[round][configuration_idx][i];
    hand_index_t group_size  = poker_data->nCr_groups[suit_size+j-i-1][j-i];
    ...
    for(; i<j-1; ++i) {
      ...
      while(low < high) {
        uint32_t mid = (low+high)/2;
        if (poker_data->nCr_groups[mid+j-i-1][j-i] <= group_index) {
          suit_index[i] = mid;
          low = mid+1;
        } else {
          high = mid;
      ...
      group_index -= poker_data->nCr_groups[suit_index[i]+j-i-1][j-i]; 
    }
  ...
  for(uint32_t i=0; i<SUITS; ++i) {
    ...
    for(uint32_t j=0; j<indexer->rounds; ++j) {
      uint32_t n              = indexer->configuration[round][configuration_idx][i]>>ROUND_SHIFT*(indexer->rounds-j-1)&ROUND_MASK;
      uint32_t round_size     = poker_data->nCr_ranks[RANKS-m][n]; m += n;
      uint32_t round_idx      = suit_index[i]%round_size; suit_index[i] /= round_size;
      uint32_t shifted_cards  = poker_data->index_to_rank_set[n][round_idx], rank_set = 0;
      for(uint32_t k=0; k<n; ++k) {
        uint32_t shifted_card = shifted_cards&-shifted_cards; shifted_cards ^= shifted_card;
        uint32_t card         = poker_data->nth_unset[used][__builtin_ctz(shifted_card)]; rank_set |= 1<<card;
