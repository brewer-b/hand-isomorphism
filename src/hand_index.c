...
static uint_fast32_t nCr_ranks[RANKS + 1][RANKS + 1], rank_set_to_index[1 << RANKS],
    index_to_rank_set[RANKS + 1][1 << RANKS], (*suit_permutations)[SUITS];
...
void enumerate_configurations_r(uint_fast32_t rounds, const uint8_t cards_per_round[], uint_fast32_t round,
                                uint_fast32_t remaining, uint_fast32_t suit, uint_fast32_t equal, uint_fast32_t used[],
                                uint_fast32_t configuration[], void (*observe)(uint_fast32_t, uint_fast32_t[], void *),
                                void *data)
{
    if (suit == SUITS)
    {
        ...
        if (round + 1 < rounds)
        {
            enumerate_configurations_r(rounds, cards_per_round, round + 1, cards_per_round[round + 1], 0, equal, used,
    ...
    else
    {
        ...
        for (uint_fast32_t i = min; i <= max; ++i)
        {
            ...
            enumerate_configurations_r(rounds, cards_per_round, round, remaining - i, suit + 1, new_equal, used,
                                       configuration, observe, data);
                              ...
                              void (*observe)(uint_fast32_t, uint_fast32_t[], void *), void *data)
{
    ...
    enumerate_configurations_r(rounds, cards_per_round, 0, cards_per_round[0], 0, (1 << SUITS) - 2, used, configuration,
                               observe, data);
}
...
out:;
    ...
    for (uint_fast32_t i = 0; i < SUITS;)
    {
        ...
        uint_fast32_t j = i + 1;
        for (; j < SUITS && configuration[j] == configuration[i]; ++j)
        {
        }
...
void enumerate_permutations_r(uint_fast32_t rounds, const uint8_t cards_per_round[], uint_fast32_t round,
                              uint_fast32_t remaining, uint_fast32_t suit, uint_fast32_t used[], uint_fast32_t count[],
                              void (*observe)(uint_fast32_t, uint_fast32_t[], void *), void *data)
{
    if (suit == SUITS)
    {
        ...
        if (round + 1 < rounds)
        {
            enumerate_permutations_r(rounds, cards_per_round, round + 1, cards_per_round[round + 1], 0, used, count,
                                     observe, data);
        }
    }
    else
    {
        ...
        for (uint_fast32_t i = min; i <= max; ++i)
        {
            ...
            enumerate_permutations_r(rounds, cards_per_round, round, remaining - i, suit + 1, used, count, observe,
                                     data);
...
bool hand_indexer_init(uint_fast32_t rounds, const uint8_t cards_per_round[], hand_indexer_t *indexer)
{
    ...
    for (uint_fast32_t i = 0; i < rounds; ++i)
    {
        ...
        if (!indexer->configuration_to_equal[i] || !indexer->configuration_to_offset[i] || !indexer->configuration[i] ||
    ...
    for (uint_fast32_t i = 0; i < rounds; ++i)
    {
        ...
        for (uint_fast32_t j = 0; j < indexer->configurations[i]; ++j)
        {
...
bool hand_unindex(const hand_indexer_t *indexer, uint_fast32_t round, hand_index_t index, uint8_t cards[])
{
    ...
    {
        uint_fast32_t j = i + 1;
        for (; j < SUITS && indexer->configuration[round][configuration_idx][j] ==
        ...
        for (; i < j - 1; ++i)
        {
            suit_index[i] = low = floor(exp(log(group_index) / (j - i) - 1 + log(j - i)) - j - i);
    ...
    for (uint_fast32_t i = 0; i < SUITS; ++i)
    {
        ...
        {
            uint_fast32_t n =
                indexer->configuration[round][configuration_idx][i] >> ROUND_SHIFT * (indexer->rounds - j - 1) &
                ROUND_MASK;
            uint_fast32_t round_size = nCr_ranks[RANKS - m][n];
            m += n;
            uint_fast32_t round_idx = suit_index[i] % round_size;
            ...
            for (uint_fast32_t k = 0; k < n; ++k)
            {
                ...
                uint_fast32_t card = nth_unset[used][__builtin_ctz(shifted_card)];
                rank_set |= 1 << card;
