...
static uint32_t nCr_ranks[RANKS + 1][RANKS + 1], rank_set_to_index[1 << RANKS],
...
void hand_index_ctor() {
  ...
  for (uint32_t i = 0; i < 1 << RANKS; ++i) {
    for (uint32_t j = 0, set = ~i & (1 << RANKS) - 1; j < RANKS;
         ++j, set &= set - 1) {
      nth_unset[i][j] = set ? ctz32(set) : 0xff;
    }
  }
  ...
  for (uint32_t i = 0; i < 1 << RANKS; ++i) {
    for (uint32_t set = i, j = 1; set; ++j, set &= set - 1) {
      rank_set_to_index[i] += nCr_ranks[ctz32(set)][j];
    }
    index_to_rank_set[popcount32(i)][rank_set_to_index[i]] = i;
  }
...
void count_permutations(uint32_t round, uint32_t count[], void *data) {
  ...
  for (uint32_t i = 0; i <= round; ++i) {
    for (uint32_t j = 0, remaining = indexer->cards_per_round[i]; j < SUITS - 1;
         ++j) {
      uint32_t size =
...
void tabulate_permutations(uint32_t round, uint32_t count[], void *data) {
  ...
  for (uint32_t i = 0; i < SUITS; ++i) {
    ...
    uint32_t smaller = popcount32((this_bit - 1) & pi_used);
...
bool hand_indexer_init(uint32_t rounds, const uint8_t cards_per_round[],
  ...
  for (uint32_t i = 0; i < rounds; ++i) {
    ...
    indexer->configuration[i] =
        calloc(indexer->configurations[i], SUITS * sizeof(uint32_t));
...
hand_index_t hand_index_next_round(const hand_indexer_t *indexer,
  ...
  for (uint32_t i = 0; i < indexer->cards_per_round[round]; ++i) {
    ...
    ranks[suit] |= rank_bit;
    shifted_ranks[suit] |=
        rank_bit >> popcount32((rank_bit - 1) & state->used_ranks[suit]);
  }

  for (uint32_t i = 0; i < SUITS; ++i) {
    assert(!(state->used_ranks[i] & ranks[i])); /* no duplicate cards */

    uint32_t used_size = popcount32(state->used_ranks[i]),
             this_size = popcount32(ranks[i]);
    state->suit_index[i] +=
  ...
  for (uint32_t i = 0, remaining = indexer->cards_per_round[round];
       i < SUITS - 1; ++i) {
    uint32_t this_size = popcount32(ranks[i]);
  ...
  for (uint32_t i = 0; i < SUITS; ++i) {
    ...
    for (uint32_t j = 0; j < indexer->rounds; ++j) {
      uint32_t n = indexer->configuration[round][configuration_idx][i] >>
                   ...
                   ROUND_MASK;
      uint32_t round_size = nCr_ranks[RANKS - m][n];
      m += n;
      ...
      for (uint32_t k = 0; k < n; ++k) {
        ...
        uint32_t card = nth_unset[used][ctz32(shifted_card)];
        rank_set |= 1 << card;
