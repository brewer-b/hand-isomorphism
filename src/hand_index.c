...
void enumerate_permutations_r(
    uint_fast32_t rounds, const uint8_t cards_per_round[], uint_fast32_t round,
    uint_fast32_t remaining, uint_fast32_t suit, uint_fast32_t used[],
    uint_fast32_t count[],
    void (*observe)(uint_fast32_t, uint_fast32_t[], void *), void *data) {
  if (suit == SUITS) {
    ...
    for (uint_fast32_t i = min; i <= max; ++i) {
      uint_fast32_t new_count =
          old_count | i << ROUND_SHIFT * (rounds - round - 1);
...
void count_permutations(uint_fast32_t round, uint_fast32_t count[],
  ...
  for (uint_fast32_t i = 0; i <= round; ++i) {
    for (uint_fast32_t j = 0, remaining = indexer->cards_per_round[i];
         j < SUITS - 1; ++j) {
      uint_fast32_t size =
...
bool hand_indexer_init(uint_fast32_t rounds, const uint8_t cards_per_round[],
  ...
  for (uint_fast32_t i = 0; i < rounds; ++i) {
    indexer->configuration_to_equal[i] =
        calloc(indexer->configurations[i], sizeof(uint_fast32_t));
    indexer->configuration_to_offset[i] =
        calloc(indexer->configurations[i], sizeof(hand_index_t));
    indexer->configuration[i] =
        calloc(indexer->configurations[i], SUITS * sizeof(uint_fast32_t));
...
hand_index_t hand_index_all(const hand_indexer_t *indexer,
  ...
  if (indexer->rounds) {
    ...
    for (uint_fast32_t i = 0, j = 0; i < indexer->rounds;
...
hand_index_t hand_index_next_round(const hand_indexer_t *indexer,
  ...
  for (uint_fast32_t i = 0; i < indexer->cards_per_round[round]; ++i) {
    ...
    uint_fast32_t rank = deck_get_rank(cards[i]),
                  suit = deck_get_suit(cards[i]), rank_bit = 1 << rank;
    assert(!(ranks[suit] & rank_bit));
    ranks[suit] |= rank_bit;
    shifted_ranks[suit] |=
        rank_bit >>
        __builtin_popcount((rank_bit - 1) & state->used_ranks[suit]);
  }

  for (uint_fast32_t i = 0; i < SUITS; ++i) {
    assert(!(state->used_ranks[i] & ranks[i])); /* no duplicate cards */

    uint_fast32_t used_size = __builtin_popcount(state->used_ranks[i]),
                  this_size = __builtin_popcount(ranks[i]);
    state->suit_index[i] +=
  ...
  for (uint_fast32_t i = 0, remaining = indexer->cards_per_round[round];
       i < SUITS - 1; ++i) {
  ...
  uint_fast32_t configuration =
      indexer->permutation_to_configuration[round][state->permutation_index];
  uint_fast32_t pi_index =
      indexer->permutation_to_pi[round][state->permutation_index];
  uint_fast32_t equal_index =
      indexer->configuration_to_equal[round][configuration];
...
bool hand_unindex(const hand_indexer_t *indexer, uint_fast32_t round,
  ...
  for (uint_fast32_t i = 0; i < SUITS; ++i) {
    ...
    for (uint_fast32_t j = 0; j < indexer->rounds; ++j) {
      uint_fast32_t n = indexer->configuration[round][configuration_idx][i] >>
      ...
      uint_fast32_t shifted_cards = index_to_rank_set[n][round_idx],
                    rank_set = 0;
