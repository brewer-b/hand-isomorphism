...
public:
    HandIsomorphism(std::vector<std::vector<uint8_t>> cardsPerRound):cardsPerRound(cardsPerRound){
        ...
        for (size_t i = 0; i < indexers.size(); i++){
            hand_indexer_init(cardsPerRound[i].size(), cardsPerRound[i].data(), &indexers[i]);
        }
    }
    ~HandIsomorphism(){
        for (size_t i = 0; i < indexers.size(); i++){
    ...
    uint64_t size(int round) const {
        return hand_indexer_size(&indexers[round], cardsPerRound[round].size() - 1);
    }
    ...
    bool unindex(int round, uint64_t index, uint8_t* output) const{
        return hand_unindex(&indexers[round], cardsPerRound[round].size() - 1, index, output);
    }
private:
